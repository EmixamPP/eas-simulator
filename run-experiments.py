...
def run_experiment_on(cpus, cpus_description):
    print(f"Stating CPUs: {cpus_description}")
        ...
        for version in versions:
            ...
            if version == EAS:
                eas_hist = (power, task_cycles, energy_cycles,
                            balance_cycles, idle_cycles, terminated_task_ratio)
            else:
    ...
    with open(f"results_{cpus_description}.csv".replace(" ", "_"), "w") as f:
        ...
        for version in versions[1:]:
            ...
            f.write("{},{},".format(np.round(mean_var["total_energy"][0], 2), np.round(
                mean_var["total_energy"][1], 2)))
            f.write("{},{},".format(
            ...
            f.write("{},{}\n".format(np.round(mean_var["terminated"][0], 2), np.round(
                mean_var["terminated"][1], 2)))

    print(f"End CPUs: {cpus_description}")
